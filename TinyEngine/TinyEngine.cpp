...
void engDrawLine(int XStart, int YStart, int XEnd, int YEnd) 
{
	SDL_SetRenderDrawColor(Renderer, CurrentColor.r, CurrentColor.g, CurrentColor.b, CurrentColor.a);
	SDL_RenderDrawLine(Renderer,XStart, YStart, XEnd, YEnd);
}

void engDrawCircle(int CX, int CY, int Radius, bool FillCircle)
{
	SDL_SetRenderDrawColor(Renderer, CurrentColor.r, CurrentColor.g, CurrentColor.b, CurrentColor.a);
	if (FillCircle == false)
	{
		...
		double Y = (double)0.5;
		double InCX = CX - 0.5;
		double InCY = CY - 0.5;//N_CY var ytter
		while (X >= Y)
		{
			SDL_RenderDrawPoint(Renderer, (int)(InCX + X), (int)(InCY + Y));
			SDL_RenderDrawPoint(Renderer, (int)(InCX + Y), (int)(InCY + X));
